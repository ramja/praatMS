...
gStats<-function(X,Intervals){
  ...
  for ( i in 1:(length(Intervals$min)-1)){
    ...
    StrengthVar<-c(StrengthVar,var(X[Intervals$max[i]:Intervals$min[i+1],'Strength'],na.rm = TRUE))
    Intensity<-c(Intensity,mean(X[Intervals$max[i]:Intervals$min[i+1],'Intensity'],na.rm = TRUE))
    IntensityVar<-c(IntensityVar,var(X[Intervals$max[i]:Intervals$min[i+1],'Intensity'],na.rm = TRUE))
  ...
  data.frame('Duration'= Duration, 'F0'=F0, 'F0Var'=F0Var,'Q4F0'=Q4F0,'MaxF0'=MaxF0,
             'F0Range'= F0Range,'Strength'=Strength,'StrengthVar'=StrengthVar,'Intensity'=Intensity,
             'IntensityVar'=IntensityVar,'Q4Intensity'=Q4Intensity,'MaxIntensity'=MaxIntensity, check.rows = TRUE, check.names = TRUE)
}
